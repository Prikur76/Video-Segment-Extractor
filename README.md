...
Файл аннотаций представляет собой JSON-файл, который содержит информацию о метках и временных интервалах для каждого видео. Пример структуры файла аннотаций:
...
[
    {
        "videoLabels": [
            {
                "timelinelabels": ["Heian-Nidan"],  # Название метки
                "ranges": [
                    {
                        "start": 100,  # Начальный кадр
                        "end": 200     # Конечный кадр
                    }
                ]
            },
            {
                "timelinelabels": ["01.KRK"],
                "ranges": [
                    {
                        "start": 500,
                        "end": 600
...
### Поля файла аннотаций

- **videoLabels**: Список меток для видео.
  - **timelinelabels**: Список названий меток (обычно одна метка).
  - **ranges**: Список временных интервалов для каждой метки.
    - **start**: Начальный кадр интервала.
    - **end**: Конечный кадр интервала.
